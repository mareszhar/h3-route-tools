...
import type { H3DuxOpenAPIDocument, ToOpenAPIOptions } from './openapi.ts'
import { writeFile } from 'node:fs/promises'
import { basename, dirname, join, resolve } from 'node:path'
import { toOpenAPI } from './openapi.ts'

/*
  Build-time tooling for the `@mszr/h3-dux/codegen` entry (Node-only, never imported at runtime):
  ...
  - writeOpenAPIDocument: emit the OpenAPI document collected by a h3-dux server.
*/
...
async function loadTypeScript(): Promise<TsModule> {
  if (!tsModule) {
    try {
      ...
      throw new Error(
        '@mszr/h3-dux/codegen: route type-gen needs the optional peer `typescript` (e.g. `npm i -D typescript`).',
      )
    }
  }
  return tsModule
...
function findLeakedNames(ts: TsModule, dts: string, options: CompilerOptions): string[] {
  ...
  for (const d of ts.getPreEmitDiagnostics(program)) {
    if (d.code !== 2304)
      continue // TS2304: Cannot find name 'X'.
...
export async function generateRoutesDts(options: GenerateRoutesOptions): Promise<string> {
  ...
  if (!source)
    throw new Error('generateRoutesDts: failed to load the flatten module.')
...
/**
 * Build a h3-dux OpenAPI document and write it to `path`, returning it.
 *
 * @example
 * import { app } from "../server"
 * await writeOpenAPIDocument(app, "openapi.json", { info: { title: "API", version: "1.0.0" } })
 */
export async function writeOpenAPIDocument(
  app: object,
  path: string,
  options: ToOpenAPIOptions & WriteOpenAPIOptions,
): Promise<H3DuxOpenAPIDocument> {
  const { indent, ...openapi } = options
  const doc = toOpenAPI(app, openapi)
  await writeFile(path, JSON.stringify(doc, null, indent ?? 2))
  return doc
}
