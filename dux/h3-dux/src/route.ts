import type { EventHandlerRequest, EventHandlerWithFetch, H3, H3Event, H3Plugin, H3RouteMeta, Middleware } from 'h3'
...
export type { RouteMethod, StatusCodeKey } from './internal/schema-types.ts'
...
export function defineRouteHandler<
  ...
  const R extends ResponseRecord<Get, Put, Post, Del, Options, Head, Patch, Trace, Connect> = ResponseRecord<Get, Put, Post, Del, Options, Head, Patch, Trace, Connect>,
>(
 ...
 * `defineRoute`/`H3DuxApp.route` mounting from a handler you already have:
...
export function mountRouteHandler(h3: H3, route: string, handler: MountableRouteHandler): void {
  ...
  for (const method of METHOD_KEYS) {
    if (isRuntimeMethod(Reflect.get(def, method)))
      h3.on(method, route, handler, opts)
  }
...
function computeAllow(event: H3Event, ownMethods: RuntimeMethods): string {
  ...
  if (route && Array.isArray(table)) {
    for (const entry of table) {
      if (entry?.route !== route || typeof entry.handler !== 'function')
        continue
  ...
  if (!harvested) {
    for (const method of METHOD_KEYS) {
      if (isRuntimeMethod(ownMethods[method]))
        allowed.add(method.toUpperCase())
    }
  }
  if (allowed.has('GET') && ownMethods.head !== false)
    allowed.add('HEAD')
  if (ownMethods.options !== false)
    allowed.add('OPTIONS')
  return [...allowed].join(', ')
}
...
function runResponseValidation(
  ...
  if (streamResponse && (streamResponse[code] ?? streamResponse[String(code)]))
    return result
