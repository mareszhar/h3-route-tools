...
import type { H3DuxApp } from './h3-app.ts'
import type { Prettify } from './internal/schema-types.ts'
import type { MountableRouteHandler, RouteHandler, RoutePlugin, RoutePluginBrand } from './route.ts'
import {
...
export type InferRoutesInput = H3DuxApp | RoutePlugin | readonly RoutePlugin[] | RouteMap

/**
 * Aggregate routes into one `{ [route]: { [method]: Endpoint } }` map — the typed-client substrate.
 * Accepts an {@link H3DuxApp} instance (`InferRoutes<typeof app>`), a single `defineRoute` plugin, a
...
export type InferRoutes<T extends InferRoutesInput>
  = T extends H3DuxApp<infer Routes>
    ? Routes
