import type { ErrorDetails, H3Event } from 'h3'
import type { ParserName } from './media-type.ts'
...
import { getQuery, getRouterParams, HTTPError } from 'h3'
import { matchMediaType, PARSER_BY_MEDIA_TYPE } from './media-type.ts'
import {
...
/** The default request-validation error: a `422` carrying the schema issues. */
...
function wrapBareJSON(
  ...
  return new Proxy(req, {
    get(target, prop: keyof H3DuxBodyRequest) {
      if (prop === 'json') {
...
async function getParsedBody(req: H3DuxBodyRequest, parser: ParserName): Promise<unknown> {
  switch (parser) {
    case 'json':
      return req.json()
    case 'formData':
...
function bindIfFn(target: H3DuxBodyRequest, prop: keyof H3DuxBodyRequest): unknown {
...
export type { BodyValidation, MediaTypeMap } from './schema-types.ts'
