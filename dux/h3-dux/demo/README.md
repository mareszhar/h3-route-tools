...
- **Response kinds** — `/motd` uses `text()` (the client gets a `string`), `/fruits/:id/label` uses `binary()` (a `Blob` download), and `DELETE /fruits/:id` returns `204` (the `empty` kind — `data` is `undefined`).
