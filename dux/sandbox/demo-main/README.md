...
> The generated module is emitted as a `.ts` (not a `.d.ts`) so its filename-truth assertions are checked even under Nitro's `skipLibCheck` — declare `params: { slug }` on `server/routes/fruits/[id].get.ts` and `tsc` reports the disagreement at the cursor. See [dux-spec-sdk.md §13](../../docs/dux-spec-sdk.md#13-nitro-deltas-via-codegen).
