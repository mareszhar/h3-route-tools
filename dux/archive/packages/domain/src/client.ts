...
import { ORCHARD_KEY, ORCHARD_KEY_HEADER } from "./config.ts";
 ...
 * shared valibot-inferred types. h3 ships no RPC client, so this shared-types
 * approach is its idiomatic story — and the h3 + Nitro-h3 demos both reuse it.
 */
export function createOrchardClient(options: OrchardClientOptions = {}) {
  ...
  async function request<T>(
    ...
    if (init.auth) headers[ORCHARD_KEY_HEADER] = key;
    ...
    async *ripen(id: string): AsyncGenerator<RipenTick> {
      ...
      for (;;) {
        ...
        while (boundary !== -1) {
          ...
          if (data) yield JSON.parse(data.slice(5).trim()) as RipenTick;
