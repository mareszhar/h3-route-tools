...
async function expectOk<T extends { ok: boolean; status: number; json: () => Promise<unknown> }>(
  ...
  if (!res.ok) throw new Error(`Unexpected ${res.status}: ${JSON.stringify(await res.json())}`);
...
export async function runHonoTrip(label: string, client: HonoClient): Promise<void> {
  ...
  if (!missing.ok) narrate.expected("error body", await errorBody(missing));
  ...
  if (!denied.ok) narrate.expected("guard rejected the write", await errorBody(denied));
  ...
  if (!broke.ok) narrate.expected("domain error", await errorBody(broke));
