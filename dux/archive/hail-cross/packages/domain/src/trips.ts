...
export async function runFetchTrip(
  ...
  if (health.ok) narrate.ok("orchard is", health.data.status);
  ...
  if (apple.ok) narrate.ok("fetched", `${apple.data.emoji} ${apple.data.name}`);

  narrate.step("GET /fruits/:id — missing → 404");
  const missing = await api.getFruit("dragonfruit");
  if (!missing.ok) narrate.expected("error body", missing.error);

  narrate.step("POST /fruits — no key → 401");
  const denied = await deniedApi.createFruit(mango);
  if (!denied.ok) narrate.expected("guard rejected the write", denied.error);
  ...
  if (patched.ok) narrate.ok("ripeness now", patched.data.ripeness);

  narrate.step("POST /checkout — out of stock → 409");
  const broke = await api.checkout({ items: [{ id: "kiwi", kg: 9999 }] });
  if (!broke.ok) narrate.expected("domain error", broke.error);
  ...
  if (receipt.ok) narrate.ok("receipt total", `$${receipt.data.total}`);
