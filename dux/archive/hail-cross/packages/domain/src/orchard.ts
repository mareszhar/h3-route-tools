...
export function createOrchard(initial: Fruit[] = seedFruits) {
  ...
  const require = (id: string): Fruit => {
    ...
    if (!fruit) throw new NotFoundError(`fruit "${id}"`);
    return fruit;
  };
  ...
  return {
    ...
    create(input: NewFruit): Fruit {
      ...
      if (fruits.has(id)) throw new ConflictError(`A fruit called "${input.name}" already exists`);
    ...
    checkout(order: CheckoutOrder): Receipt {
      const lines = order.items.map(({ id, kg }) => {
        const fruit = require(id);
        if (fruit.stockKg < kg) throw new OutOfStockError(fruit.name, kg, fruit.stockKg);
    ...
    async *ripen(
      ...
      while (fruit.ripeness < 100) {
        ...
        if (fruit.ripeness < 100) await sleep(intervalMs);
      }
    },
  };
}
