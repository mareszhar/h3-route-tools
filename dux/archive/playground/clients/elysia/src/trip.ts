...
export async function runElysiaTrip(label: string, api: ElysiaClient): Promise<void> {
  ...
  if (missing.error?.status === 404)
    narrate.expected('narrowed error.value', missing.error.value)
  ...
  if (broke.error?.status === 409)
    narrate.expected('domain error', broke.error.value)
