...
import type { Serialize, Simplify } from "./internal/serialize.ts";
 ...
 * the response is reported through {@link Serialize}, then {@link Simplify}d so it hovers as the concrete
 * wire shape rather than a `Serialize<…>` wrapper.
 */
type ResponseOf<E> = E extends { response: infer R } ? Simplify<Serialize<R>> : unknown;
